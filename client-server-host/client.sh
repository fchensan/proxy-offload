...
then
	...
	ssh $PROXY_USERNAME@$PROXY_ADDR "~/proxy-offload/proxy-host/proxy-setup.sh tcp nginx ~/proxy-offload/proxy-host/nginx-conf"
	...
	do
		...
		nohup ~/iperf/src/iperf3 -c $PROXY_ADDR -f m -p $PORT -P $STREAMS_PER_PROCESS -b $TARGET_BITRATE -i 60 -t $DURATION --timestamp > logs-$PROCESSES/iperf3-$PORT.json 2> logs-$PROCESSES/iperf3-$PORT.err &
		sleep $SPAWN_NEW_PROCESS_RATE
	done
	echo "$SERVER_PORT_END/$SERVER_PORT_END [Done]"

	echo "Letting experiment run for another $DURATION second(s). It's currently $(date)"
	sleep $DURATION 
	echo "Done!"
	...
	ssh $PROXY_USERNAME@$PROXY_ADDR "sudo pkill monitor.sh"
	sleep 2
	ssh $PROXY_USERNAME@$PROXY_ADDR "cat ~/proxy-offload/proxy-host/data.log" > monitor.log
	ssh $PROXY_USERNAME@$PROXY_ADDR "sudo cat /var/log/nginx/error.log" > nginx-error.log
	ssh $PROXY_USERNAME@$PROXY_ADDR "sudo nginx -s quit"
fi
